define utils::bash_exec (
  ...
  # The multiple bash wrappers cause us to lose the cwd.
  if $cwd != undef {
    $cded_command = "cd ${cwd} && (${command})"
  } else {
    $cded_command = $command
  }

  $escaped_command = regsubst($cded_command, "\"", "\\\"", 'G')
